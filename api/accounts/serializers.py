...
from accounts.models import Language,ClientProfile,ContactUs
...
class ContactUsCreateSerializer(serializers.ModelSerializer):
    ...
    # Timestamp is automatically generated by backend
    ...
    class Meta:
        ...
        fields = [
            ...
            "honeypot", "timestamp", "attachment", "tags"
        ]

        read_only_fields = [
            "ip_address", "user_agent", "referrer",
            "priority", "tracking_id", "submission_token"
        ]

    # VALIDATIONS

    def validate_honeypot(self, value):
        """Reject bots filling the hidden honeypot."""
        if value:
            raise serializers.ValidationError("Spam detected.")
        return value

    def validate_name(self, value):
        """Basic name checks + avoid uppercase spam."""
        value = value.strip()
    ...
    def validate_message(self, value):
        ...
        spam_patterns = [
            ...
            r"\b(viagra|cialis|casino|porn|sex)\b",
            # Too many emails
            r"[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}"
        ]

        for pattern in spam_patterns:
            if len(re.findall(pattern, value, re.IGNORECASE)) > 2:
                raise serializers.ValidationError(
    ...
    def validate_email(self, value):
        ...
        if domain in disposable:
            raise serializers.ValidationError("Disposable email not allowed.")
    ...
    def create(self, validated_data):
        """Override default create() to add metadata."""

        # Auto timestamp
        validated_data["timestamp"] = timezone.now()

        # Remove honeypot before saving
